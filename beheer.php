...
<head>
    ...
    <style>
        ...
        .icon-dropdown-menu .dropdown-item i {
    font-size: 1.3rem;
    margin-right: 8px;
    vertical-align: middle;
}
.icon-dropdown-btn .icon-preview i {
    font-size: 1.3rem;
    vertical-align: middle;
    margin-right: 6px;
}
    ...
    <form method="post" class="row g-2 mb-3">
        ...
        <div class="col">
            <div class="icon-dropdown">
                <input type="hidden" name="icoon" required>
                <button type="button" class="btn btn-outline-secondary dropdown-toggle icon-dropdown-btn" data-bs-toggle="dropdown" aria-expanded="false">
                    <span class="icon-preview"></span> Kies een icoon...
                </button>
                <ul class="dropdown-menu icon-dropdown-menu" style="max-height:300px;overflow:auto;">
                    <?php foreach($bootstrap_icons as $icon): ?>
                        <li>
                            <a href="#" class="dropdown-item icon-option" data-value="<?= $icon ?>">
...
document.querySelectorAll('.icon-option').forEach(function(option){
    option.addEventListener('click', function(e){
        ...
        const dropdown = this.closest('.icon-dropdown');
        dropdown.querySelector('input[name="icoon"]').value = value;
        const preview = dropdown.querySelector('.icon-preview');
        preview.innerHTML = `<i class="bi ${value}"></i>`;
        const dropdownMenu = dropdown.querySelector('.icon-dropdown-menu');
